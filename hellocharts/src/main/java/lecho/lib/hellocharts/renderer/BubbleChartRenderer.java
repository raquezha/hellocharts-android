...
public class BubbleChartRenderer extends AbstractChartRenderer {
    ...
    public void removeMargins() {
        ...
        final float scaleX = computator.getMaxViewport().width() / contentRect.width();
        final float scaleY = computator.getMaxViewport().height() / contentRect.height();
        float dx = 0;
        float dy = 0;
        ...
        Viewport maxViewport = computator.getMaxViewport();
    ...
    private void drawLabel(Canvas canvas, BubbleValue bubbleValue, float rawX, float rawY) {
        ...
        float top = rawY - (labelHeight + 0f) / 2 - labelMargin;
        float bottom = rawY + (labelHeight + 0f) / 2 + labelMargin;
