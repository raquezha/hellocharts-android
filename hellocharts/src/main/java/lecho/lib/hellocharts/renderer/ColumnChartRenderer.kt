...
public class ColumnChartRenderer extends AbstractChartRenderer {
    ...
    public void onChartViewportChanged() {
        if (isViewportCalculationEnabled) {
            ...
            computator.setCurrentViewport(computator.getMaxViewport());
        }
    }
    ...
    private void drawColumnsForSubcolumns(Canvas canvas) {
        ...
        int columnIndex = 0;
        for (Column column : data.getColumns()) {
            processColumnForSubcolumns(canvas, column, columnWidth, columnIndex, MODE_DRAW, data.isRoundedCorner());
            ++columnIndex;
        }
    }
    ...
    private void checkTouchForSubcolumns(float touchX, float touchY) {
        ...
        for (Column column : data.getColumns()) {
            ...
            processColumnForSubcolumns(null, column, columnWidth, columnIndex, MODE_CHECK_TOUCH, data.isRoundedCorner());
            ++columnIndex;
        }
    }
    ...
    private void processColumnForSubcolumns(Canvas canvas, Column column, float columnWidth, int columnIndex, int mode, boolean isRoundedCorners) {
        ...
        for (SubcolumnValue columnValue : column.getValues()) {
            ...
            switch (mode) {
                case MODE_DRAW:
                    drawSubcolumn(canvas, column, columnValue, false, isRoundedCorners);
                    break;
                case MODE_HIGHLIGHT:
                    highlightSubcolumn(canvas, column, columnValue, valueIndex, false, isRoundedCorners);
                    break;
    ...
    private void drawColumnForStacked(Canvas canvas) {
        ...
        int columnIndex = 0;
        for (Column column : data.getColumns()) {
            processColumnForStacked(canvas, column, columnWidth, columnIndex, MODE_DRAW, data.isRoundedCorner());
            ++columnIndex;
        }
    }
    ...
    private void checkTouchForStacked(float touchX, float touchY) {
        ...
        for (Column column : data.getColumns()) {
            // canvas is not needed for checking touch
            processColumnForStacked(null, column, columnWidth, columnIndex, MODE_CHECK_TOUCH, data.isRoundedCorner());
            ++columnIndex;
        }
    }
    ...
    private void processColumnForStacked(Canvas canvas, Column column, float columnWidth, int columnIndex, int mode, boolean isRoundedCorners) {
        ...
        for (SubcolumnValue columnValue : column.getValues()) {
            ...
            switch (mode) {
                case MODE_DRAW:
                    drawSubcolumn(canvas, column, columnValue, true, isRoundedCorners);
                    break;
                case MODE_HIGHLIGHT:
                    highlightSubcolumn(canvas, column, columnValue, valueIndex, true, isRoundedCorners);
                    break;
                case MODE_CHECK_TOUCH:
                    checkRectToDraw(columnIndex, valueIndex);
                    break;
    ...
    private void drawSubcolumn(Canvas canvas, Column column, SubcolumnValue columnValue, boolean isStacked, boolean isRoundedCorners) {
        if(isRoundedCorners) {
        ...
        if (selectedValue.secondIndex == valueIndex) {
            ...
            if(isRoundedCorners) {
                canvas.drawRoundRect(drawRect.left - touchAdditionalWidth, drawRect.top, drawRect.right + touchAdditionalWidth,
                        drawRect.bottom, 100f, 100f, columnPaint);
            } else {
