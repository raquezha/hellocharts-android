...
class ChartZoomer(zoomType: ZoomType) {
    val zoomer: ZoomerCompat = ZoomerCompat()
    ...
    fun scale(computator: ChartComputator, focusX: Float, focusY: Float, scale: Float): Boolean {
        ...
        val left = viewportFocus.x - (focusX - computator.contentRectMinusAllMargins.left) *
            ( newWidth / computator.contentRectMinusAllMargins.width())
        val top = viewportFocus.y + (focusY - computator.contentRectMinusAllMargins.top) *
            (newHeight / computator.contentRectMinusAllMargins.height())
